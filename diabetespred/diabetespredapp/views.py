...
from diabetespredapp.main import train # train() which trains the data model in main.py
  # startup() which predicts the outcome
from diabetespredapp.main1 import startup
...
def prediction(request):
    if request.method == 'POST':
        ...
        skin = int(request.POST.get('Skin'))
        ...
        result = startup(pregnancy, glucose, bp, skin, insulin, bmi, dpf, age)
        if result == 1:
...
def training(request):
    ...
    fig.savefig('diabetespredapp/static/accuracyplot.jpg')
